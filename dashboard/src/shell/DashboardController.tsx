...
export function DashboardController() {
  ...
  if (status.loading) return <EmptyState title="Checking dashboard API" message={`Connecting to ${client.baseUrl}.`} />;
  if (status.error) {
    const title = status.error.kind === "version-mismatch" ? "Dashboard API version mismatch" : "Dashboard API offline";
    return <ErrorState title={title} message={status.error.message} onAction={status.reload} />;
  }
  if (!status.data?.data.ready) {
    return <ErrorState title="Dashboard API not ready" message="The local API is not ready yet." onAction={status.reload} />;
  }
