...
export function RequestRow({
  ...
  const artifactCount = "artifact_count" in request ? request.artifact_count : request.artifact_inclusions.length;
  const timestamp = "timestamp" in request ? request.timestamp : undefined;
  ...
  return (
    ...
    >
      ...
      <dl className="request-metrics" aria-label="Request token totals">
        {requestMetricEntries(request.usage, artifactCount).map((entry) => (
