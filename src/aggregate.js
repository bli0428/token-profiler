export function aggregateEvents(events) {
  const usageEvents = events.filter((event) => event.event_kind === "request_usage");
  const artifactEvents = events.filter((event) => event.event_kind !== "request_usage");
  ...
  for (const event of sortedEvents) {
    const tokenCount = Number(event.token_count) || 0;
