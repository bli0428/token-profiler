...
- [X] T007 Convert src/profiler.js, src/tokenizer.js, and src/hash.js to thin compatibility re-exports
...
**Goal**: Live Codex proxy behavior remains stable while Codex-specific implementation lives under the Codex proxy ingestor.

**Independent Test**: `node --import tsx --test test/proxy.test.js`

- [X] T009 [US1] Move Codex session routing from src/session-router.js into src/ingest/codex-proxy/session-router.ts
- [X] T010 [US1] Move Codex config mutation from src/codex-config.js into src/ingest/codex-proxy/config.ts
- [X] T011 [US1] Update src/ingest/codex-proxy/server.ts and src/surfaces/cli/proxy-commands.ts imports to use domain-owned Codex proxy modules
- [X] T012 [US1] Convert src/session-router.js and src/codex-config.js to thin compatibility re-exports
...
**Goal**: Codex log import and Codex session metadata reading live under a Codex log import ingestor; CLI only orchestrates.

**Independent Test**: `node --import tsx --test test/codex-sessions.test.js`

- [X] T014 [US2] Create src/ingest/codex-log-import/index.ts for Codex rollout import and session metadata reading
- [X] T015 [US2] Move readCodexSessionMetadata and enrichProfilerSessions from src/codex-sessions.js into src/ingest/codex-log-import/index.ts
- [X] T016 [US2] Move runCodexImport parsing behavior out of src/surfaces/cli/capture-commands.ts into src/ingest/codex-log-import/index.ts
- [X] T017 [US2] Update src/surfaces/cli/capture-commands.ts and src/surfaces/cli/report-commands.ts to delegate to src/ingest/codex-log-import/index.ts
- [X] T018 [US2] Convert src/codex-sessions.js to a thin compatibility re-export
...
- [X] T020 [US3] Create fixture-only source adapter in src/ingest/fixture-source/index.ts
...
- [X] T023 [US4] Add architecture-boundary checks that analysis modules do not import any src/ingest source module in test/architecture-boundaries.test.js
- [X] T024 [US4] Add architecture-boundary checks that source-root compatibility modules remain thin re-exports in test/architecture-boundaries.test.js
- [X] T025 [US4] Document allowed source adapter exports in src/ingest/fixture-source/index.ts and src/ingest/codex-proxy/index.ts
...
- [X] T026 [US5] Ensure src/profiler.js, src/session-router.js, src/codex-config.js, src/codex-sessions.js, src/hash.js, and src/tokenizer.js are thin compatibility re-exports only
- [X] T027 [US5] Update src/index.js exports to point at domain-owned modules while preserving public API names
- [X] T028 [US5] Search for remaining imports of moved implementation modules and replace with domain-owned imports where internal code is not relying on compatibility wrappers
...
2. Move Codex live proxy support into `src/ingest/codex-proxy/`.
3. Move Codex log import support into `src/ingest/codex-log-import/`.
